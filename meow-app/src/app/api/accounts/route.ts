  ...
  try {
    ...
    if (customer) {
      ...
      return NextResponse.json(
        { error: `Failed to create account number after ${ACCOUNT_CREATION_RETRY_LIMIT} attempts` },
        { status: 400 },
      );
    } else {
...
function generateAccountNumber(): string {
  const groups = Array.from({ length: 4 }, () => {
    const digits = Array.from({ length: 4 }, () => crypto.randomInt(0, 10)).join("");
    return digits;
  });
...
async function getCustomerById(id: number) {
  if (id) {
    const supabase = await createClient();
    const { data } = await supabase.from("customers").select("id").eq("id", id).single();
